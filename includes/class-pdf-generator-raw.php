...
class Quote_Manager_PDF_Generator_Raw
{
    ...
    public function generate($quote_id)
    {
        ...
        // Check if quote is rejected - for API calls, just return empty string
        $status = get_post_meta($quote_id, '_quote_status', true);
        if ($status === Quote_Manager_System_For_Woocommerce::STATUS_REJECTED) {
            // Only return empty if this is a direct download request, not for email generation
            if (!isset($_GET['email_generation']) && !isset($_POST['email_generation'])) {
                return '';
            }
        }
