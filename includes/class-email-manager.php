...
class Quote_Manager_Email_Manager
{
    ...
    public function send_quote_email($quote_id, $custom_subject = '', $custom_message = '')
    {
        ...
        try {
            ...
            // Check if quote is rejected before generating PDF
            $status = get_post_meta($quote_id, '_quote_status', true);
            if ($status === Quote_Manager_System_For_Woocommerce::STATUS_REJECTED) {
                error_log('Quote is rejected, not generating PDF: ' . $quote_id);
                return ['success' => false, 'message' => 'Quote has been rejected.'];
            }
