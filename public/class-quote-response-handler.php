...
class Quote_Manager_Response_Handler {
    ...
    public function register_endpoints() {
        ...
        if (!get_option('quote_manager_endpoints_flushed')) {
            flush_rewrite_rules(false);
            update_option('quote_manager_endpoints_flushed', true);
        }
    }
    ...
    private function generate_signed_pdf($quote_id) {
        ...
        $pdf_path = $pdf_dir . 'SIGNED_QUOTE_' . $quote_id . '.pdf';
        
        file_put_contents($pdf_path, $pdf_output);
        
        // Save the signed PDF path
        update_post_meta($quote_id, '_signed_quote_pdf_path', $pdf_path);
    }
